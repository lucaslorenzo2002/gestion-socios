...
export class CuotasDAO {
    ...
    async eliminarAllSocioCuotaByInscripcionInCategoria(actividadId, categoriaId, clubAsociadoId) {
        try {
            const socios = await this.transaccionesDAO.sociosWithTransaccionesPendientesCuotaInscripcion(null, actividadId, categoriaId, clubAsociadoId);
    ...
    async findAllCuotasProgramadasWithAcciones(diaActual) {
        try {
    ...
    async actualizarMontoSocioCuotasVencidas(cuotaProgramadaId, montoVencimiento, interes) {
        try {
    ...
    async findCuotaByTipoAndPeriodo(periodo, tipoDeSocioId, actividadId, categoriaId, clubAsociadoId) {
        try {
            if (tipoDeSocioId) {
                return await Cuota.findOne({
                    where: {
    ...
    async getSocioCuotaInscripcion(inscripcionId, socioId) {
        try {
            return await Socio_Cuota.findOne({
                where: {
                    inscripcion_id: inscripcionId,
    ...
    async getLast3CuotasPagas(socioId, clubAsociadoId) {
        try {
            const socio = await this.sociosDAO.getSocioById(socioId); //(o (1))
            const titularFamiliaId = socio.Grupo_familiar?.dataValues.familiar_titular_id;
            if (socio.dataValues.grupo_familiar_id && socio.dataValues.id !== titularFamiliaId) {
                return [];
            }
