...
import { ICuota } from '../interfaces/ICuota.js';
...
import { ICuotaProgramada } from '../interfaces/ICuotaProgramada.js';
...
export class CuotasApi{
	...
	jobsMap: Map<any, any>;
	deudaGrupoFamiliar: Map<any, any>;
	inscripcionesApi: InscripcionesApi;
	sociosDAO: SociosDAO;
	transaccionesApi: TransaccionesApi;
	actualizarCutasVencidasJobsMap: Map<any, any>;
	constructor(){
	...
	async asignarCuotaAUnSocio(socioId: number, tipoSocioId: number, actividadId: number, categoriaId: number, clubAsociadoId: number){
		let mesActual:any = new Date().getMonth()+1;
		...
		if(socio.dataValues.grupo_familiar_id){
			const grupoFamiliar:any = await this.gruposFamiliaresApi.getGrupoFamiliarById(socio.dataValues.grupo_familiar_id);
	...
	async asignarCuotaASocios(cuotaParam, club){
		try {
			const cuota:any = await this.cuotasDAO.findCuotaById(cuotaParam.dataValues.id);
	...
	async programarCuota(
		...
		diaDeVencimientoParam: number,
		actualizaMontoCuandoVence: boolean,
		frecuenciaInteres: string,
		montoPostVencimiento: number,
		interesCuota: number,
		club
		){
		try {
	...
	async cronJobCuota(cuotaProgramadaId: number){
	...
	async actualizarCuotasVencidas(){
		cron.schedule('0 3 10-28 * *', async() => {
			...
			const cuotasProgramadasConAcciones:any = await this.cuotasDAO.findAllCuotasProgramadasWithAcciones(diaActual);
		...
		mesesAbonados: number 
			...
			if(tipoDeCuota !== 'cuota inscripcion'){
				...
				if(inscripcion.dataValues.actividad_id){
					...
					let categoriaId:any = null;
					
					if(transaccion.dataValues.detalles.includes('categoriaId')){
						await this.transaccionesApi.aprobarTransaccion(
							transaccion.dataValues.id, 
							transaccionesEnum.inscripcionDeportiva,
							true,
							socioCuota.dataValues.socio_id,
							transaccion.dataValues.detalles,
							clubAsociado
						)
					...
					await this.transaccionesApi.aprobarTransaccion(
						transaccion.dataValues.id, 
						transaccionesEnum.inscripcionSocial,
						false,
						socioCuota.dataValues.socio_id,
						transaccion.dataValues.detalles,
						clubAsociado
